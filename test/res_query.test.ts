...
const testCases = Object.keys(st).map(key => [
  ...
  [`Is ${ss[key].to} ${key}`, ss[key].from - 1, "is", key, false],
  [`Is ${ss[key].from} ${key}`, ss[key].from - 1, "is", key, true],
  [`Is ${ss[key].to - 1} ${key}`, ss[key].from - 1, "is", key, true],
  ...
  [`${key} and smaller ${ss[key].to}`, ss.Desktop.to, key, "andSmaller", true],
  [
...
function testCase(width, propOne, propTow, expected) {
  ...
  expect(resQuery[propOne][propTow]).toEqual(expected);
}
